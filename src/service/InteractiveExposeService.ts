...
export class InteractiveExposeService extends APIClient {
    ...
    async changeLogo(type: 'light' | 'dark', image: File): Promise<S3File> {
        ...
        return (
            await this.invokeApi(`/settings/logos/${type}`, 'POST', formData, {
    ...
    async createInteractiveExposeTemplate(templateName: string, body: FormData) {
        return this.invokeApi('/interactiveExposes/templates/' + templateName, 'POST', body, {
            headers: { 'Content-Type': 'multipart/form-data' },
        });
    }

    async uploadInteractiveExposeTemplateFiles(templateName: string, body: FormData) {
        return this.invokeApi('/interactiveExposes/templates/' + templateName, 'PUT', body, {
            headers: { 'Content-Type': 'multipart/form-data' },
        });
    }

    async deleteInteractiveExposeTemplate(templateName: string) {
        return this.invokeApi('/interactiveExposes/templates/' + templateName, 'DELETE');
    }
    ...
    async fetchInteractiveExposeTemplateFileData(templateName: string, fileType: string, body: string[]) {
        return this.invokeApi(`/public/interactiveExposes/templates/${templateName}/fileType/${fileType}/content`, 'POST', body, {
            headers: { 'Content-Type': 'application/json' },
        });
    }
