...
const buildSortParameter = (sorting: Sort): string | undefined => {
  ...
  return sorting?.fields
    ?.map(
      (fieldName) => `${sorting.direction === "ASC" ? "+" : "-"}${fieldName}`
    )
    .join(",");
};
...
export default class MatchController extends APIClient {
  ...
  async fetchMatchesBySearchProfile(
    ...
    return await this.invokeApiWithErrorHandling<
      PagedResponse<MatchmakingTypes.Match>
    >(`/match/search-profile/${searchProfileId}`, "GET", undefined, {
      queryParams: {
        ...query,
        size,
        offset,
  ...
  async fetchMatchesBySearchProfileBody(
    ...
    return await this.invokeApiWithErrorHandling<
      PagedResponse<MatchmakingTypes.Match>
    >(`/match/search-profile`, "POST", searchProfile, {
      queryParams: {
        ...query,
        size,
        offset,
  ...
  async fetchMatchesByEstate(
    ...
    return await this.invokeApiWithErrorHandling<
      PagedResponse<MatchmakingTypes.Match>
    >(`/match/estate/${estateId}`, "GET", undefined, {
      queryParams: {
        ...query,
        size,
        offset,
  ...
  async fetchMapMatchesBySearchProfile(searchProfileId: string) {
    return await this.invokeApiWithErrorHandling<
      PagedResponse<MatchmakingTypes.MapMatch>
    >(`match/search-profile/${searchProfileId}/map-entries`, "GET");
  }
}
