...
var HttpClient = function () {
    function HttpClient(apiMapping) {
        ...
        this.setAPIURL = function () {
            if (_this.apiClient) {
                var baseUrl = _this.stageToUse === 'local' ? 'http://localhost:8080' : 'https://cloudios.' + _this.stageToUse + '.flowfact.cloud/edge-service';
                _this.apiClient.config.url = baseUrl + '/' + _this.serviceName + '/' + _this.apiVersionTag;
            }
        };
